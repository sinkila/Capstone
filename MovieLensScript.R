...
if(!require(LaTeX)) install.packages("LaTeX", repos = "https://www.latex-project.org/get/")
...
# The least and most common
temp <- edx %>%
  group_by(genres) %>%
  summarize(count = n()) %>%
  arrange(count)

slice_head(temp, n= 10)
slice_tail(temp, n= 10)


#Analyzing the genre types there were two genres from outside the standard (IMAX and "no genres listed)
...
#Analyzing which movies have no genre listed or IMAX as genre
temp <- edx %>%
  filter(genres == "(no genres listed)" | genres == "IMAX") %>%
  select(movieId, title, genres) %>%
  arrange(desc(movieId))

slice_tail(temp , n= 21)

#=> Conclusion: As there are very few "IMAX" and "(no genres listed)" and 
#both fall under the category "Short movies", they will be  updated to new category "Short"
...
##Conclusion: Movie ratings seem to be impacted by the genres. So, we will include it in the model.
...
temp_d %>%
  ...
  geom_point() +
  geom_smooth()+
  ggtitle("Average rating per date reviewed")

##Conclusion: There does not seem to be a significant impact on the timestamp of when the rating was given.
# So timestamp will be ignored in the model development
...
##Conclusion: There seems to be a trend that the older the movie the lower the rating. 
#We'll include the year_pub in the model development. 
...
removed <- anti_join(test_set, validation)
train_set <- rbind(train_set, removed)

#Due to the performance issues, I'm save files so I don't need to recreated
saveRDS(train_set, file = "MovieLens_train_set.RDS") 
#train_set<- readRDS("MovieLens_train_set.RDS")

saveRDS(test_set, file = "MovieLens_test_set.RDS") 
#test_set<- readRDS("MovieLens_test_set.RDS")

#Remove extra object for performance sake while developing the model
#rm(test_index, removed)
#rm(edx, validation) #not needed while building the model


##Building on the simple models used in the course 8 
#to get understanding of the differences, if any

#We use the same function as used in the course 8 to compute the RMSE for vectors of ratings and their corresponding predictors:
...
# I'll be using approximation of mu and biases as for performance reasons, I cannot use the lm function: fit <- lm(rating ~ as.factor(movieId), data = train_set)
...
#Conclusion: We can see that these estimates vary.
...
##=> Conclusion: Just using the Genres -column (modified) data with LSE did not improve the results significantly

## Adding year of publishing the movie
#Computing an approximation of year of publishing the movie + movie + user + genre effects using the modified year_pub -column
...
#Testing out how penalized least squares behaves in the projects data set with the model 
#including publishing year, movie, user and genre biases

#Choosing the penalty terms using cross validation
lambdas <- seq(0, 10, 0.25)

#Using the same structure for the sapply as used in the course 8 to help follow the code
...
#Adding the results as a new row to the RMSE tibble
rmse_results <- bind_rows(rmse_results,
                          data_frame(method="Regularized Publishing year + Movie + User + Genres Effect Model",  
                                     RMSE = min(rmses)))
rmse_results

#Saving the RMSE
saveRDS(RMSE, file = "MovieLens_RMSE-2021-02-12-2.RDS") 
